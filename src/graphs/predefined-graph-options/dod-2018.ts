import { GraphData, ValueColumn } from '../components/types';
...
const xColumn: Column = {
  name: 'Companies',
  valueType: 'string',
  values: []
};

const yColumn: ValueColumn = {
...
data.forEach(dat => {
  xColumn.values.push(dat[0]);
  yColumn.values.push(dat[1] as number);
});

export const defenseGraphData: GraphData = {
  graphType: 'basic',
  graphTitle: defenseOptions.title?.text,
  seriesData: [
    {
      seriesType: 'column',
